...
\paragraph{}
L’implémentation du réseau à été pensée de la manière suivante : chaque neurones et chaque synapses sont représenté par un acteur. Un synchronizer, qui est également  un agent, est en charge de récupérer tous les messages, et de les redistribuer à travers le réseau pour éviter tout problème de causalité temporelle.

\paragraph{}
Les neurones sont réparties dans différentes couche. Comme le simulateur pour le moment suit le modèle restreins de machine de Boltzmann, les seul communication entre les neurones se font entre des couche voisine. Il n'y a donc pas non plus d’interconnexion entre des neurones d'une même couche.

\paragraph{}
Avec le modèle actuelle, chaque neurones possède un potentiel qui, lorsqu'il dépasse un seuil fixé, va envoyer un spike à l'ensemble des neurones qui sont relié avec lui. Chaque synapse possède un poids synaptique qui varie au cour de la simulation en fonction des spike qui affecte ou non le neurones dans une certaine fenêtre de temps. Pour reproduire ce comportement, différent type de spike ont été crée : 
\begin{itemize}
\item \emph{PreSynapticSpike} qui est le spike envoyé par les neurones vers un synapse
\item \emph{PostSynapticForwardSpike} qui est le spike envoyé par une synapse vers un neurone. Ce dernier transmet sont propre poids synaptique au neurone.
\item \emph{PostSynapticBackwardSpike} qui est le spike renvoyé par le neurones vers ses synapse lorsque sont potentiel à dépassé le seuil fixé. Celui ci est nécéssaire pour affecter le poids synaptique en fonction de la réaction du neurone.
\end{itemize}
